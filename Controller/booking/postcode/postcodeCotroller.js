...
const Locations = require("../../../model/locations/locationsSchema");
...
exports.getCoveredCitiesFromLocations = async (req, res) => {
    try {
        const locations = await Locations.find({});
        const coveredCities = new Set();

        locations[0].locations.forEach(location => {
            // Check if the location name contains multiple cities separated by slashes
            if (location.name.includes('/')) {
                // Split the location name by slashes and trim each part
                const splitLocations = location.name.split('/').map(city => city.trim());
                // Add each individual city to the set with first letter capitalized
                splitLocations.forEach(city => {
                    const capitalizedCity = city.charAt(0).toUpperCase() + city.slice(1).toLowerCase();
                    coveredCities.add(capitalizedCity);
                });
            } else {
                // Add the location name with first letter capitalized
                const capitalizedCity = location.name.charAt(0).toUpperCase() + location.name.slice(1).toLowerCase();
                coveredCities.add(capitalizedCity);
            }
        });

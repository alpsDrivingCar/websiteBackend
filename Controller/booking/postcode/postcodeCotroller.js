...
exports.getCoveredCities = async (req, res) => {
    try {
        // Find all active instructors accepting students
        ...
        if (instructors.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'No active instructors found accepting students'
            });
        }

        // Map of postcode prefixes to city names
        const postcodeToCityMap = {
            'AB': 'Aberdeen',
            'AL': 'St Albans',
            'B': 'Birmingham',
            'BA': 'Bath',
        ...
        // Extract all areas covered by instructors
        const allAreas = instructors.map(instructor => instructor.areas).flat();
        
        // Get unique areas
        const uniqueAreas = [...new Set(allAreas)];
        
        // Map areas to cities where possible
        const coveredCities = new Set();
        
        uniqueAreas.forEach(area => {
            // Try to extract the postcode prefix (first 1-2 letters)
            const prefix = area.match(/^[A-Z]{1,2}/i)?.[0]?.toUpperCase();
            
            if (prefix && postcodeToCityMap[prefix]) {
                coveredCities.add(postcodeToCityMap[prefix]);
            }
        });

        console.log("Covered Cities: ", Array.from(coveredCities));
        return res.status(200).json({
            success: true,
            data: Array.from(coveredCities).sort()
        });
    } catch (error) {
